...
void Chip8::_instr8XY5(u8 X, u8 Y) {
    if(_V[X] > _V[Y])
        _V[0xF] = 1;
    else _V[0xF] = 0;
    _V[X] -= _V[Y];
...
void Chip8::_instr8XY7(u8 X, u8 Y) {
    if(_V[X] < _V[Y])
        _V[0xF] = 1;
    else _V[0xF] = 0;
    _V[X] = _V[Y] - _V[X];
...
void Chip8::_instr8XYE(u8 X) {
    _V[0xF] = _V[X] & 0x80;
    _V[X] *= 2;
}

/* Skip next instruction if Vx != Vy. */
void Chip8::_instr9XY0(u8 X, u8 Y) {
    if(_V[X] != _V[Y])
        _PC += 2;
}

/* Set the value of the register I to NNN. */
void  Chip8::_instrANNN(u16 NNN) {
    _I = NNN;
}

/* Jump to address NNN + V0. */
void Chip8::_instrBNNN(u16 NNN) {
    _PC = NNN + _V[0];
}

/* Generate a random u8 and AND it with KK. The result is stored in Vx. */
void Chip8::_instrCXKK(u8 X, u8 KK) {
    u8 r = m_randGen() % 255;
    _V[X] = r & KK;
}

/* Display n-u8 sprite starting at memory location I at (Vx, Vy), set VF = collision. */
void Chip8::_instrDXYK(u8 X, u8 Y, u8 K) {
...
void Chip8::_instrFX1E(u8 X) {
    _I += _V[X];
}

/* Set I to the location of the sprite for the digit stored in Vx. */
void Chip8::_instrFX29(u8 X) {
    _I = _V[X] * 5;
}

/* Store BCD representation of Vx in memory locations I, I+1, and I+2. */
void Chip8::_instrFX33(u8 X) {
    _memory[_I] = _V[X] / 100;
...
void Chip8::_instrFX55(u8 X) {
    for(int i = 0 ; i != 0x10; ++i)
        _memory[_I + i] = _V[i];
}

/* Read registers V0 through Vx from memory starting at location I. */
void Chip8::_instrFX65(u8 X) {
    for(int i = 0 ; i != 0x10; ++i)
        _V[i] = _memory[_I + i];
}
